...
stdenv.mkDerivation rec {
  ...
  postPatch = ''
    ...
    substituteInPlace scripts/e_gen_menu --replace "Prefixes += [getenv('HOME') + '/.local']" "Prefixes += [getenv('HOME') + '/.local', '/run/current-system/sw', getenv('HOME') + '/.nix-profile']"
    
    substituteInPlace scripts/* --replace "/usr/bin/python3" "/usr/bin/env python3"
  '';
